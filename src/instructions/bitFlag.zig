...
pub const bitFlag = struct {
    ...
    pub fn test_bit_r8(proc: *Processor, bit: Bit, registerValue: *u8) void {
...
test "bitFlag.test_bit_hl_indirect" {
    const HL: u16 = 0x31E7;
    var memory = Memory.init();
    ...
    bitFlag.test_bit_hl_indirect(&processor, .seven);
    try expectEqual(0, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);

    bitFlag.test_bit_hl_indirect(&processor, .six);
    try expectEqual(0, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);

    bitFlag.test_bit_hl_indirect(&processor, .five);
    try expectEqual(0, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);

    bitFlag.test_bit_hl_indirect(&processor, .four);
    try expectEqual(0, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);

    bitFlag.test_bit_hl_indirect(&processor, .three);
    try expectEqual(1, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);

    bitFlag.test_bit_hl_indirect(&processor, .two);
    try expectEqual(1, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);

    bitFlag.test_bit_hl_indirect(&processor, .one);
    try expectEqual(1, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);

    bitFlag.test_bit_hl_indirect(&processor, .zero);
    try expectEqual(1, processor.flags.zero);
    try expectEqual(0, processor.flags.negative);
    try expectEqual(1, processor.flags.half_carry);
}
